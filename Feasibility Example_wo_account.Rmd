...
```{r}
# Finding earliest appearance on a continent for a specific species (with cleaned data)
...
for (continent in names(continents)) {
  find_earliest_appearance_species(tomato_d2, continent, continents[[continent]], species_name)
}
```

In the project, it will be coded that the earliest apperance year can be
taken over directly from the last chunk and no year numbers need to be
given in the following chunk. And also that the sequence of the
continents is from the oldest to the youngest apperance year.
